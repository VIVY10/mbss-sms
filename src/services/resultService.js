...
async function deleteResult(data) {
  const result = await resultModel.deleteResult(data);
...
async function updateResult(data) {
  const result = await resultModel.updateResult(data);
...
async function getMissingMarks(teacherid, classid, subjectcode, examid) {
  return resultModel.getMissingMarks(teacherid, classid, subjectcode, examid);
}
...
async function processStudentMarks(examid, subjectCode, marks, entered_by) {
  ...
  try {
    ...
    const validMarks = marks.filter(
      (m) => m.studentclassid && m.mark !== undefined,
    );
    ...
    const validation = await resultModel.validateAllStudentsCanEnterMarks(
      examid,
      subjectCode,
      studentIds,
      connection,
    ...
    const toInsert = validMarks.filter(
      (m) => !existingIds.has(m.studentclassid),
    );
    const toUpdate = validMarks.filter((m) =>
      existingIds.has(m.studentclassid),
    );
...
async function getClassResults({
  ...
  if (!allocationExists) {
    throw Object.assign(
      new Error("You are not allocated to this class and subject."),
      { status: 403 },
    );
  }
